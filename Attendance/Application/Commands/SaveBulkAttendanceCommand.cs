...
public class SaveBulkAttendanceCommand
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!SaturdayCalendar.IsSaturday(request.Date))
        {
            return Result.Failure<IReadOnlyList<AttendanceRecordDto>>(
                $"La asistencia solo se registra los sábados. '{request.Date:yyyy-MM-dd}' es {TraducirDia(request.Date.DayOfWeek)}.");
        }
        ...
        await _attendance.UpsertBulkAsync(records, cancellationToken);
    ...
    private static string TraducirDia(DayOfWeek day) => day switch
    {
